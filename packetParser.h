...
unsigned int rtpParser(unsigned char *raw , int rtpLength);
unsigned int fecParser(unsigned char *raw , int fecLength);

unsigned int rtpParser(unsigned char *raw , int rtpLength) {
    unsigned int raw_offset = 0;
    unsigned int paysize;
    /*unsigned char payload[MAX_BUF_SIZE];*/
    struct rtpHeader rtp_h;
    ...
    if(rtp_h.pt == 96) {
        fecParser(raw + raw_offset , paysize);
    }
